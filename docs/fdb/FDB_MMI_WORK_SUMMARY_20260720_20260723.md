...
## 17. 7 月 23 日续作：在线 Semantic Overlay Shadow 接入

在离线 `semantic-overlay-v4-frozen` 通过 holdout v2 后，已开始把该策略接入
实时 Agent。当前仍严格保持 Shadow，不接管 `interrupt`、`commit` 或 `clear`。

| 项目 | 当前结果 | 中文说明 |
|---|---:|---|
| 在线观察器 | 已实现 | 新增 `mmi_turn_detection/semantic_overlay.py`，接收流式 ASR 和基础 MMI 决策。 |
| 控制权 | 未改变 | 基础 MMI 和 LiveKit 继续执行原动作，overlay 只输出反事实建议。 |
| 语义请求 | 异步、无重试 | 避免同步阻塞实时控制链路，也避免一次超时被多次重试放大。 |
| 并发保护 | 已实现 | 并发槽满时直接记录 `semantic_overlay_dropped`，不让旧请求排队。 |
| 延迟观测 | 已实现 | 记录 API 延迟、是否早于基础决策到达、是否落在 300 ms 假设生效窗口内。 |
| 故障降级 | fail-open | 缺少配置、请求超时、HTTP 错误或解析错误均不改变 Agent 行为。 |
| 在线日志分析器 | 已实现 | 自动汇总成功率、P50/P95、300 ms 到达率、丢弃率和动作变化。 |
| 本地回归 | 233 passed | 覆盖 MMI、FDB replay、语义 v3.2、overlay v4、在线分析器、留出集工具和 SpeakerGate。 |

新增结构化日志：

| 日志事件 | 中文说明 |
|---|---|
| `semantic_overlay_comparison` | 基础动作与当前 overlay 建议的实时对比。 |
| `semantic_overlay_result` | 语义分类、置信度、请求延迟和是否会改变基础动作。 |
| `semantic_overlay_dropped` | 并发槽已满，当前请求因来不及进入实时窗口而丢弃。 |
| `semantic_overlay_error` | 请求超时、HTTP 或返回格式异常；仅记录，不影响会话。 |
...
下一步不是直接开启 Active，而是在服务器 Shadow Worker 中启用该观察器，先跑

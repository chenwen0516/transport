...
- `FDB_MMI_SEMANTIC_HYBRID_V3_2_P11_20260723.json`
- `FDB_MMI_SEMANTIC_HYBRID_V3_2_PER20_20260723.json`

冻结留出集已按 `en_qwen_shadow_mmi_holdout_v1_20260723` 启动一次性采集。
完成后只补充最终结果，不再根据留出集错误修改 v3.2。

采集任务由 systemd 单元 `fdb-mmi-holdout-v1-20260723.service` 托管。最后一次
可观测进度为 48/80；随后 `113.44.234.195` 的 22/80/443/7880 端口均从当前
公网出口不可达。保留集结果因此暂未回收，48/80 不是最终成绩。服务器入口恢复后
应先检查该单元和 run 目录；若任务中断，使用同一 run-id 加 `--resume` 接续，
不得重选样本或修改冻结候选。
